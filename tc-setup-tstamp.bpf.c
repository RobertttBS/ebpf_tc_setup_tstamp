...
{
    unsigned long now = bpf_ktime_get_tai_ns(), delay = 2000; // 1ms default
    delay = (skb->priority) ? skb->priority : delay;
    delay <<= 10; // convert to microseconds
    delay += 15000000; // 15~16 micro seconds in my computer is the minimum delay required to make it work
